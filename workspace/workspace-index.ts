...
const PROTO_SURFACE_PHASE: Record<string, string> = {
...
function renderContextoSesion(): void {
  const host = document.querySelector<HTMLElement>("[data-home-context]");
  if (!host) return;
...
function renderScenarioCatalog(): void {
  const body = document.querySelector<HTMLTableSectionElement>("[data-scenario-rows]");
  if (!body) return;
  ...
  PROTO_SCENARIOS.forEach((scenario) => {
    ...
    if (scenario.scenarioId === activo) row.className = "is-current";
...
document.addEventListener("DOMContentLoaded", () => {
  if (!protoIsAuthenticated()) return; // el shell ya habrá redirigido
