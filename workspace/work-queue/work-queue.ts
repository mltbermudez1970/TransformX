...
function protoQueueItems(): ProtoWorkItem[] {
  const base = protoGetWorkQueue();
  if (!protoQueueRefrescada) return base;
...
function protoRenderWorkQueue(): void {
  const host = document.querySelector<HTMLElement>("[data-work-queue]");
  if (!host) return;
...
function protoWireQueueRefresh(): void {
  document.querySelector<HTMLButtonElement>("[data-queue-refresh]")?.addEventListener("click", () => {
    protoQueueRefrescada = true;
    protoRenderWorkQueue();
    // El foco va al encabezado de la lista para anunciar el cambio de contenido.
    document.querySelector<HTMLElement>("#queue-list-title")?.setAttribute("tabindex", "-1");
    document.querySelector<HTMLElement>("#queue-list-title")?.focus();
  });
}
 ...
 */
function protoScheduleIncomingWork(): void {
  if (protoQueueRefrescada) return;
  ...
  window.setTimeout(() => {
    if (!protoQueueRefrescada) banner.hidden = false;
  }, 2500);
}
