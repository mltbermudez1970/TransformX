...
interface ProtoConversionOutcome {
  result: ProtoConversionResult;
  conflictCode?: ProtoConversionConflictCode;
  message: string;
  /** Estado autoritativo refrescado que la UI debe adoptar. */
  refreshRequired: boolean;
  retryAllowed: boolean;
  latestLeadVersion?: number;
  availableActions?: ProtoAction[];
  readiness?: ProtoOpportunityReadiness;
  changedBy?: { displayName: string; role: ProtoRole };
  changedAt?: string;
  existingOpportunity?: { opportunityReference: string; status: ProtoOpportunityStatus };
  opportunity?: ProtoQualifiedOpportunity;
  referenceId?: string;
}

/** Rama de revalidación forzada por URL, para recorrer los desenlaces. */
function protoForcedRevalidation(): string | null {
  return protoUrlParam("revalidate");
}

/** Readiness alternativo devuelto cuando la revalidación detecta cambio. */
function protoReadinessChangedFor(leadId: string): ProtoOpportunityReadiness | null {
  ...
  return {
    ...
    conditions: base.conditions.map((c) =>
      c.conditionId === "RC-07"
        ? {
            ...(rc07 ?? c),
            status: "BLOCKING" as ProtoReadinessStatus,
 ...
 */
async function protoRevalidateAndConvert(lead: ProtoCanonicalLead): Promise<ProtoConversionOutcome> {
