...
function protoOppLeadId(): string {
...
function protoRenderReadinessCondition(c: ProtoReadinessCondition): string {
...
function protoRenderOpp01(host: HTMLElement, lead: ProtoCanonicalLead): void {
...
function protoRenderOpp02(host: HTMLElement, lead: ProtoCanonicalLead): void {
  const r = protoGetReadiness(lead.leadId);
...
function protoWireConversion(lead: ProtoCanonicalLead): void {
  const btn = document.querySelector<HTMLButtonElement>("[data-conv-submit]");
  const feedback = document.querySelector<HTMLElement>("[data-conv-feedback]");
  const conflictHost = document.querySelector<HTMLElement>("[data-conv-conflict]");
  if (!btn || !feedback || !conflictHost) return;
  ...
  btn.addEventListener("click", async () => {
    ...
    switch (out.result) {
      ...
      case "CONCURRENCY_CONFLICT":
        ...
        btn.disabled = true;
        conflictHost.querySelector<HTMLElement>("h2")?.setAttribute("tabindex", "-1");
        conflictHost.querySelector<HTMLElement>("h2")?.focus();
        return;
...
function protoRenderSys06(out: ProtoConversionOutcome, lead: ProtoCanonicalLead): string {
  ...
  return `
    <section class="ws-section ws-sys06" aria-labelledby="sys06-title" data-sys06>
      ...
      ${out.availableActions
        ? `<p class="ws-muted ws-small">Acciones disponibles tras el refresco: ${out.availableActions.map((a) => protoEsc(protoActionLabel(a))).join(" · ")}.${
            out.availableActions.includes("CONVERT_TO_OPPORTUNITY") ? "" : " <strong>Convertir ya no está entre ellas.</strong>"
          }</p>`
        : ""}
...
function protoRenderOpp03(host: HTMLElement, lead: ProtoCanonicalLead): void {
  ...
  if (!opp && refExterna) {
    const r = protoGetReadiness(lead.leadId);
    if (r) opp = protoCreateOpportunity(lead, r);
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const host = document.querySelector<HTMLElement>("[data-opportunity-surface]");
  if (!host) return;
  ...
  if (view === "convert") protoRenderOpp02(host, lead);
  else if (view === "handoff") protoRenderOpp03(host, lead);
  else protoRenderOpp01(host, lead);
});
