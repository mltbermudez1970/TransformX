...
function protoRenderPolicyCriteria(c: ProtoAssignmentCandidate): string {
...
function protoRenderCandidates(exc: ProtoAssignmentException): string {
...
function protoRenderAssignmentException(host: HTMLElement, exc: ProtoAssignmentException): void {
  ...
  protoRenderDecisionWorkspace(host, {
    ...
    ai: {
      type: "AI_SUGGESTION",
      taskId: ai.taskRef,
      summary: `${ai.summary} Recomienda a ${recomendado ? recomendado.displayName : ai.recommendedCandidateId}${
        recomendado && recomendado.eligibility.status !== "ELIGIBLE"
    ...
    onResolved: (r, rationale, extra) => {
      ...
      const evento: ProtoAssignmentHistoryEventType =
        r.code === "OVERRIDE_ASSIGNMENT_POLICY" ? "ASSIGNMENT_OVERRIDE_APPROVED"
...
function protoRenderAssignmentHistory(host: HTMLElement, leadId: string): void {
...
function protoRenderHistoryRecord(h: ProtoAssignmentHistoryRecord): string {
  ...
  return `
    <li class="ws-history__item ws-history__item--${h.eventType.toLowerCase()}">
      ...
      <dl class="ws-history__meta">
        ...
        ${h.fromOwner || h.toOwner ? `<div><dt>Responsable</dt><dd>${
          h.fromOwner ? `${protoEsc(h.fromOwner.displayName)} → ` : ""
        }${h.toOwner ? protoEsc(h.toOwner.displayName) : "<span class='ws-muted'>sin asignar</span>"}</dd></div>` : ""}
        ${h.queue ? `<div><dt>Cola</dt><dd>${protoEsc(h.queue)}</dd></div>` : ""}
        <div><dt>Motivo</dt><dd><code>${protoEsc(h.reasonCode)}</code>${
          fallo ? " — resultado de negocio, no error técnico" : ""
        }</dd></div>
        ${h.rationale ? `<div><dt>Justificación</dt><dd>${protoEsc(h.rationale)}</dd></div>` : ""}
        ${h.policy ? `<div><dt>Política</dt><dd><code>${protoEsc(h.policy.policyVersion)}</code>${
          h.policy.violatedCriteria?.length
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const host = document.querySelector<HTMLElement>("[data-assignment-surface]");
  if (!host) return;
