...
function protoAdmLeerBorrador(userId: string): ProtoAccessDraft | null {
  try {
    const raw = sessionStorage.getItem(`${PROTO_ADM_DRAFT_KEY}:${userId}`);
    return raw ? (JSON.parse(raw) as ProtoAccessDraft) : null;
  } catch {
    return null;
  }
}

function protoAdmGuardarBorrador(userId: string, draft: ProtoAccessDraft): void {
  try {
...
function protoAdmBorrarBorrador(userId: string): void {
  try {
...
function protoRenderAdm06(host: HTMLElement, tenantId: string): void {
...
function protoRenderAdm0708(host: HTMLElement, tenantId: string, userId: string): void {
...
function protoWireAdm08(userId: string, tenantId: string, draft: ProtoAccessDraft): void {
  const form = document.querySelector<HTMLFormElement>("[data-adm-form]");
  const review = document.querySelector<HTMLButtonElement>("[data-adm-review]");
  const reset = document.querySelector<HTMLButtonElement>("[data-adm-reset]");
  if (!form || !review || !reset) return;
  ...
  form.querySelectorAll<HTMLInputElement>("[data-adm-bizcap-toggle]").forEach((cb) => {
    cb.addEventListener("change", () => {
      const biz = cb.getAttribute("data-adm-bizcap-toggle");
      if (!biz) return;
      const roles = form.querySelectorAll<HTMLInputElement>(`[data-adm-bizcap="${CSS.escape(biz)}"]`);
      if (cb.checked) {
        draft[biz] = draft[biz] ?? [];
  ...
  form.querySelectorAll<HTMLInputElement>("[data-adm-role]").forEach((cb) => {
    cb.addEventListener("change", () => {
      const biz = cb.getAttribute("data-adm-bizcap");
      const rol = cb.getAttribute("data-adm-role") as ProtoRole | null;
      if (!biz || !rol) return;
  ...
  form.querySelectorAll<HTMLInputElement>("[data-adm-tenant-role]").forEach((cb) => {
    cb.addEventListener("change", () => {
      const rol = cb.getAttribute("data-adm-tenant-role") as ProtoRole | null;
      if (!rol) return;
...
function protoRenderAdm09(host: HTMLElement, tenantId: string, userId: string): void {
  const u = PROTO_AUTH_USERS.find((x) => x.userId === userId);
  ...
  const cambios = set.changes
    .map((c) => {
      const etiqueta =
        c.kind === "BIZCAP_ADDED" ? "Añade BizCap"
...
function protoWireAdm09(
  userId: string,
  tenantId: string,
  draft: ProtoAccessDraft,
  set: ProtoAccessChangeSet
): void {
  const btn = document.querySelector<HTMLButtonElement>("[data-adm-confirm]");
  const feedback = document.querySelector<HTMLElement>("[data-adm-feedback]");
  if (!btn || !feedback) return;

  btn.addEventListener("click", async () => {
    // Step-up simulado: mismo contrato que AUTH-12, sin proveedor real.
    if (set.requiresStepUp) {
      const otp = document.querySelector<HTMLInputElement>("#adm-otp");
    ...
    const acceso = protoEffectiveAccess(userId, tenantId);
    const sec = document.querySelector<HTMLElement>("[data-adm-result]");
    const body = document.querySelector<HTMLElement>("[data-adm-result-body]");
    if (sec && body) {
      body.innerHTML = `
      ...
      sec.hidden = false;
      btn.disabled = true;
      document.querySelector<HTMLElement>("#adm-result-title")?.focus();
    }
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  const host = document.querySelector<HTMLElement>("[data-adm-surface]");
  if (!host) return;
  ...
  if (!userId) protoRenderAdm06(host, tenantId);
  else if (view === "review") protoRenderAdm09(host, tenantId, userId);
  else protoRenderAdm0708(host, tenantId, userId);
});
