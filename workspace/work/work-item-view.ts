...
function protoEsc(texto: string): string {
  const d = document.createElement("div");
  d.textContent = texto;
  return d.innerHTML;
}

function protoFormatFecha(iso: string | null): string {
...
function protoRenderPriorityBadge(code: ProtoPriorityCode): string {
...
function protoRenderStatusBadge(status: ProtoWorkItemStatus): string {
...
function protoPrimaryAction(item: ProtoWorkItem): ProtoActionDefinition | null {
  const materiales = item.availableActions
    .map((a) => protoFindAction(a))
    .filter((a): a is ProtoActionDefinition => a !== null && a.material);
  if (materiales.length > 0) return materiales[0] ?? null;
 ...
 */
function protoRenderWorkItem(item: ProtoWorkItem, variante: "row" | "card"): string {
...
function protoRenderWorkList(items: ProtoWorkItem[], vacioTexto: string): string {
  if (items.length === 0) {
 ...
 */
function protoWireWorkListNavigation(surface: string): void {
  document.querySelectorAll<HTMLElement>("[data-work-item]").forEach((art) => {
    art.querySelectorAll<HTMLAnchorElement>("a").forEach((a) => {
 ...
 */
function protoRenderSys08IfResolved(host: HTMLElement, workItemId: string | null): boolean {
