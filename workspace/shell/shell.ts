...
function protoEscape(texto: string): string {
...
function protoRenderTrustBanner(): string {
  return `
...
function protoRenderNavNode(node: ProtoNavNode, actual: string, nivel: number): string {
...
function protoRouteEnRama(actual: string, routeId: string): boolean {
  const r = protoFindRoute(actual);
  if (!r) return false;
  return r.parents.includes(routeId);
}

function protoRenderBreadcrumbs(): string {
...
function protoRenderAccountMenu(user: ProtoAuthUser | null): string {
  if (!user) {
    // En las propias superficies de identidad el CTA sería redundante.
    if (protoCurrentRouteId() === "auth") return "";
...
function protoRenderShell(): void {
  const mount = document.querySelector<HTMLElement>(PROTO_SHELL_MOUNT);
  if (!mount) return;
...
function protoWireShell(): void {
  /**
   * Menú de cuenta con el patrón ARIA de `role="menu"`: tabindex móvil,
   * navegación con flechas, Home/End, Escape y retorno de foco.
   */
  const toggle = document.querySelector<HTMLButtonElement>(".ws-account__toggle");
  const panel = document.querySelector<HTMLElement>("#ws-account-panel");
  if (toggle && panel) {
    ...
    const cerrar = (devolverFoco: boolean): void => {
      panel.hidden = true;
    ...
    const abrir = (alFinal = false): void => {
      panel.hidden = false;
    ...
    toggle.addEventListener("click", () => {
      if (toggle.getAttribute("aria-expanded") === "true") cerrar(true);
      else abrir();
    });
    ...
    panel.addEventListener("keydown", (e) => {
      ...
      switch (e.key) {
        case "ArrowDown": e.preventDefault(); enfocar(actual + 1); break;
        case "ArrowUp": e.preventDefault(); enfocar(actual - 1); break;
        case "Home": e.preventDefault(); enfocar(0); break;
        case "End": e.preventDefault(); enfocar(lista.length - 1); break;
    ...
    document.addEventListener("click", (e) => {
      ...
      const t = e.target;
      if (t instanceof Node && !panel.contains(t) && !toggle.contains(t)) cerrar(false);
    });
  }

  document.querySelector<HTMLButtonElement>("[data-ws-signout]")?.addEventListener("click", () => {
    protoClearSession();
    location.assign(protoRouteHref("auth", { screen: "sign-in" }));
  });

  document.querySelector<HTMLButtonElement>("[data-ws-expire]")?.addEventListener("click", () => {
  ...
  const navToggle = document.querySelector<HTMLButtonElement>(".ws-nav-toggle");
  const navHost = document.querySelector<HTMLElement>("[data-workspace-nav]");
  if (navToggle && navHost) {
    const cerrarNav = (devolverFoco: boolean): void => {
    ...
    navToggle.addEventListener("click", () => {
      ...
      if (abierto) navHost.querySelector<HTMLElement>("a")?.focus();
    });
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape" && navHost.classList.contains("is-open")) cerrarNav(true);
    });
  }
}
 ...
 */
function protoGuardRoute(): boolean {
  const route = protoCurrentRoute();
  if (!route || !route.requiresAuth) return true;

  if (protoIsAuthenticated()) return true;
