...
function protoRenderTenantBadge(): string {
...
function protoRenderTenantOptions(): string {
  const user = protoGetSessionUser();
  if (!user) return "";
 ...
 */
function protoRenderEffectiveAccessSummary(user: ProtoAuthUser): string {
  const tenantId = protoGetActiveTenantId();
  if (!tenantId) return `<p class="ws-account__meta">Sin organización activa.</p>`;
 ...
 */
function protoMostrarCambioDeTenant(destinoNombre: string): void {
  ...
  document
    .querySelectorAll<HTMLButtonElement>("button[data-conv-submit], button[data-dw-submit], button[data-lead-command], button[type=submit]")
    .forEach((b) => { b.disabled = true; });
}

/** Cablea el conmutador con el patrón de menú ya usado en el shell. */
function protoWireTenantSwitcher(): void {
  const toggle = document.querySelector<HTMLButtonElement>("#ws-tenant-trigger");
  const panel = document.querySelector<HTMLElement>("#ws-tenant-panel");
  ...
  document.addEventListener("click", (e) => {
    ...
    if (!panel.contains(t) && !toggle.contains(t)) cerrar(false);
  });

  panel.querySelectorAll<HTMLButtonElement>("[data-tenant-go]").forEach((b) => {
