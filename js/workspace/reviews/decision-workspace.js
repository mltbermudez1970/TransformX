...
function protoRenderEvidenceList(items, categoria) {
    if (items.length === 0) {
    ...
    <li class="ws-evidence__item ws-evidence__item--${e.category.toLowerCase()}">
      ...
      <p class="ws-evidence__src">${protoEsc(PROTO_EVIDENCE_SOURCE_LABEL[e.sourceType])}${e.sourceRef ? ` · ${protoEsc(e.sourceRef)}` : ""}${e.confidence !== null ? ` · confianza ${String(Math.round(e.confidence * 100))}%` : ""}</p>
    </li>`).join("")}</ul>`;
}
function protoRenderAiBlock(ai) {
    return `
    <section class="ws-section ws-ai" aria-labelledby="dw-ai-title">
      ...
      <p class="ws-muted ws-small">${protoEsc(ai.type)} · ${protoEsc(ai.taskId)} · autoridad ${protoEsc(ai.authority)}${typeof ai.confidence === "number" ? ` · confianza ${String(Math.round(ai.confidence * 100))}%` : ""}</p>
    </section>`;
}
function protoRenderResolutionOptions(m) {
    return `
...
function protoRenderDecisionWorkspace(host, m) {
    host.innerHTML = `
...
function protoWireDecisionWorkspace(m) {
    const submit = document.querySelector("[data-dw-submit]");
    const feedback = document.querySelector("[data-dw-feedback]");
    const rationale = document.querySelector("#dw-rationale");
    const reqMark = document.querySelector("[data-dw-req]");
    const extraWrap = document.querySelector("[data-dw-extra]");
    const extraSel = document.querySelector("#dw-extra");
    const extraHint = document.querySelector("[data-dw-extra-hint]");
    const dialog = document.querySelector("#dw-confirm");
    if (!submit || !feedback || !rationale || !dialog)
        return;
    const elegida = () => {
        const sel = document.querySelector('input[name="dw-res"]:checked');
        if (!sel)
            return null;
    ...
    const refrescar = () => {
        ...
        if (!r)
            return;
        if (reqMark)
            reqMark.hidden = !r.requiresRationale;
        if (extraWrap)
            extraWrap.hidden = !r.requiresAdditionalInput;
        if (extraHint)
            extraHint.textContent = r.additionalInputSchema?.label ?? "";
        dialog.classList.toggle("c-dialog--destructive", r.destructive);
    };
    document.querySelectorAll('input[name="dw-res"]').forEach((i) => {
    ...
    submit.addEventListener("click", () => {
        ...
        const desc = document.querySelector("#dw-confirm-desc");
        const note = document.querySelector("#dw-confirm-note");
        if (desc)
            desc.textContent = `Vas a aplicar: ${r.label}.`;
        if (note)
            note.textContent = r.consequence;
        dialog.showModal();
        dialog.querySelector("[data-dw-confirm]")?.focus();
    });
    dialog.querySelector("[data-dw-cancel]")?.addEventListener("click", () => {
        dialog.close();
        submit.focus();
    });
    ...
    dialog.querySelector("[data-dw-confirm]")?.addEventListener("click", async (e) => {
        ...
        const resultSec = document.querySelector("[data-dw-result]");
        const resultBody = document.querySelector("[data-dw-result-body]");
        ...
        document.querySelectorAll('input[name="dw-res"]').forEach((i) => { i.disabled = true; });
        ...
        document.querySelector("#dw-result-title")?.focus();
    });
    refrescar();
}
