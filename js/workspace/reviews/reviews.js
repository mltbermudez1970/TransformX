 ...
 * ========================================================================= */
function protoRenderHumanReview(host, review) {
    ...
    protoRenderDecisionWorkspace(host, {
        ...
        onResolved: (r) => `
      <p class="ws-muted ws-small">Se conserva la evaluación original del criterio ${criterio ? `${protoEsc(criterio.id)} (<strong>${protoEsc(PROTO_CRITERION_STATUS_LABEL[criterio.status])}</strong>)` : ""} y su evidencia. La resolución <strong>${protoEsc(r.label)}</strong> se registra como decisión humana separada.</p>`,
    });
...
function protoRenderDuplicateComparison(review) {
    ...
    <tr class="ws-dup__row ws-dup__row--${c.similarity.classification.toLowerCase()}">
      ...
      <td><span class="ws-sim ws-sim--${c.similarity.classification.toLowerCase()}">${protoEsc(PROTO_SIMILARITY_LABEL[c.similarity.classification])}</span>${c.similarity.score !== null ? `<br><span class="ws-muted ws-small">IA ${String(Math.round(c.similarity.score * 100))}%</span>` : ""}</td>
    </tr>`).join("");
...
function protoRenderDuplicateReview(host, review) {
...
document.addEventListener("DOMContentLoaded", () => {
    if (!protoIsAuthenticated())
        return;
    const host = document.querySelector("[data-reviews-surface]");
    if (!host)
        return;
