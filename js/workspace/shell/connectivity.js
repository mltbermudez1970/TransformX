...
function protoMaterialButtons() {
    return [
        ...document.querySelectorAll("[data-conv-submit], [data-dw-submit], [data-dw-confirm], [data-mi-send], [data-mi-resolve], [data-lead-command], [data-qual-command]"),
    ];
}
function protoRenderConnectivityBanner() {
    if (document.querySelector("[data-connectivity]"))
        return;
    ...
    banner.querySelector("[data-reconnect]")?.addEventListener("click", () => {
        protoSetOffline(false);
    });
}
function protoSetOffline(offline) {
    ...
    const pendientes = document.querySelectorAll("[data-offline-disabled]");
    if (pendientes.length > 0) {
      ...
      <button type="button" class="c-btn c-btn--primary" data-offline-refresh>Actualizar</button>`;
        ...
        aviso.querySelector("[data-offline-refresh]")?.addEventListener("click", () => location.reload());
    }
}
function protoInitConnectivity() {
