...
function protoActivateScenario(scenarioId) {
    ...
    if (user) {
        ...
        if (tenantId)
            protoSetActiveTenant(tenantId);
    }
    location.assign(protoResolveRoute(s.initialRoute));
}
function protoRenderScenarioPanel() {
...
function protoWireScenarioSelector() {
    const dialog = document.querySelector("#ws-scenario-dialog");
    const trigger = document.querySelector("[data-scenario-open]");
    if (!dialog || !trigger)
        return;
    trigger.addEventListener("click", () => {
        dialog.showModal();
        dialog.querySelector("[data-scenario-go]")?.focus();
    });
    dialog.querySelector("[data-scenario-close]")?.addEventListener("click", () => dialog.close());
    dialog.addEventListener("close", () => trigger.focus());
    dialog.querySelectorAll("[data-scenario-go]").forEach((b) => {
        b.addEventListener("click", () => {
            const id = b.getAttribute("data-scenario-go");
            if (id)
                protoActivateScenario(id);
        });
    });
    dialog.querySelector("[data-scenario-reset]")?.addEventListener("click", () => {
        protoResetScenarioState();
        location.reload();
    });
}
