 ...
 */
const PROTO_MI_DRAFT_BASE = "Hola, gracias por tu interés. Para continuar con la evaluación necesitamos confirmar la cantidad mensual estimada y la unidad de medida. ¿Podrías indicárnoslas?";
let protoMiDraftDescartado = false;
let protoMiDraftVersion = 1;
function protoMiVista() {
    return protoUrlParam("view") === "received" ? "received" : "missing";
}
/* ===========================================================================
 * MI-01 — Información faltante
 * ========================================================================= */
function protoRenderMi01(host, lead) {
...
function protoMiEtiquetaCampo(campo) {
    const mapa = {
...
function protoWireMi01(lead) {
    const feedback = document.querySelector("[data-mi-feedback]");
    const draftHost = document.querySelector("[data-mi-draft]");
    const rewire = () => {
        if (draftHost)
            draftHost.innerHTML = protoRenderMiDraft();
        protoWireMi01(lead);
    };
    document.querySelector("[data-mi-regenerate]")?.addEventListener("click", () => {
        protoMiDraftDescartado = false;
        protoMiDraftVersion += 1;
        rewire();
        document.querySelector("#mi-draft")?.focus();
    });
    document.querySelector("[data-mi-discard]")?.addEventListener("click", () => {
        protoMiDraftDescartado = true;
        rewire();
        document.querySelector("[data-mi-regenerate]")?.focus();
    });
    document.querySelector("[data-mi-send]")?.addEventListener("click", async (e) => {
        const btn = e.currentTarget;
        if (!feedback)
            return;
        const texto = document.querySelector("#mi-draft")?.value.trim() ?? "";
        if (texto === "") {
 ...
 * ========================================================================= */
function protoRenderMi03(host, lead) {
...
function protoRenderComparacion(c) {
    const val = (v, u) => v === null ? "<span class='ws-missing'>Sin valor</span>" : `${protoEsc(String(v))}${u ? ` ${protoEsc(u)}` : ""}`;
    return `
...
function protoWireMi03(lead, comparaciones) {
    const feedback = document.querySelector("[data-mi-feedback]");
    document.querySelectorAll("[data-mi-resolve]").forEach((btn) => {
        btn.addEventListener("click", async () => {
            const field = btn.getAttribute("data-mi-resolve");
            if (!field || !feedback)
                return;
            const elegido = document.querySelector(`input[name="res-${field}"]:checked`);
            if (!elegido)
                return;
            ...
            if (aplicado && cmp) {
                protoRenderFeedback(feedback, {
                    severidad: "success",
                    mensaje: `Resolución aplicada: ${PROTO_MI_ACTION_LABEL[elegido.value]}.`,
                    detalle: elegido.value === "ACCEPT_RECEIVED_VALUE"
...
/* ========================================================================= */
function protoRenderMiCabecera(lead, titulo, codigo) {
...
document.addEventListener("DOMContentLoaded", () => {
    if (!protoIsAuthenticated())
        return;
    const host = document.querySelector("[data-mi-surface]");
    if (!host)
        return;
    // SYS-08: si el WorkItem ya no está activo, se reemplaza en línea.
    if (protoRenderSys08IfResolved(host, protoUrlParam("workItemId")))
        return;
    ...
    if (protoMiVista() === "received")
        protoRenderMi03(host, lead);
    else
        protoRenderMi01(host, lead);
});
