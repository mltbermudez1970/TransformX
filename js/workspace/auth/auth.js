...
function protoAuthParam(nombre) {
    return new URLSearchParams(location.search).get(nombre);
}
function protoAuthScreenId() {
...
function protoAuthForcedOutcome() {
...
function protoAuthFeedback(mensaje, tipo) {
    const zona = document.querySelector("[data-auth-feedback]");
    if (!zona)
        return;
 ...
 */
function protoAuthLimpiarFeedback() {
    const zona = document.querySelector("[data-auth-feedback]");
    if (!zona)
        return;
...
function protoAuthMarcarCampo(input, invalido) {
...
function protoAuthLanding() {
    ...
    const extra = { screen: "select-organization" };
    const ret = protoAuthParam("returnTo");
    if (ret)
        extra["returnTo"] = ret;
 ...
 */
function protoAuthCampo(id, label, type, autocomplete, ayuda) {
...
function protoRenderGuionUsuarios() {
    const filas = PROTO_AUTH_USERS.map((u) => `<tr><th scope="row">${u.email}</th><td>${u.guion}</td></tr>`).join("");
    return `
...
function protoWireSignIn() {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        const desenlace = forzado ?? (user ? user.scriptedOutcome : "auth_failure");
        const res = await protoMockRequest({
 ...
 * ------------------------------------------------------------------------- */
function protoRenderForgot() {
    return `
    <form class="auth-form" data-auth-form novalidate>
      ${protoAuthCampo("email", "Correo corporativo", "email", "username", "Si existe una cuenta asociada, recibirás instrucciones. No confirmamos si un correo está registrado.")}
...
function protoWireForgot() {
    ...
    form.addEventListener("submit", async (e) => {
        ...
        protoAuthFeedback("Si ese correo corresponde a una cuenta, enviamos instrucciones para restablecer la contraseña. Revisa tu bandeja. (Simulado: no se envía ningún correo.)", "ok");
        form.hidden = true;
...
function protoWirePassword(etiqueta, destino) {
    const form = document.querySelector("[data-auth-form]");
    const btn = document.querySelector("[data-auth-submit]");
    if (!form || !btn)
        return;
    form.addEventListener("submit", async (e) => {
        e.preventDefault();
        const nueva = form.querySelector("#nueva");
        const conf = form.querySelector("#confirmar");
        if (!nueva || !conf)
            return;
...
function protoWireLocked() {
    document.querySelector("[data-auth-unlock]")?.addEventListener("click", async (e) => {
...
function protoRenderInvitation() {
    return `
    <div class="auth-note" role="note">
 ...
 * ------------------------------------------------------------------------- */
function protoRenderCampoOtp(label) {
    return `
    <div class="auth-field">
...
function protoRenderMfaEnrollment() {
    const user = protoGetSessionUser();
...
function protoRenderMfaChallenge() {
    const user = protoGetSessionUser();
    const policy = user ? protoPolicyOf(user) : null;
    return `
...
function protoRenderMfaRecovery() {
    return `
    <div class="auth-note" role="note">
...
function protoRenderTrustedDevice() {
    const user = protoGetSessionUser();
 ...
 */
function protoRenderSelectOrganization() {
    const user = protoGetSessionUser();
    if (!user) {
...
function protoWireSelectOrganization() {
    document.querySelectorAll("[data-tenant-enter]").forEach((b) => {
        b.addEventListener("click", () => {
            const tenantId = b.getAttribute("data-tenant-enter");
            if (!tenantId)
                return;
            if (!protoSetActiveTenant(tenantId)) {
                protoAuthFeedback("Ese acceso ya no está disponible. Vuelve a iniciar sesión para actualizar tus organizaciones.", "error");
                return;
            }
...
function protoWireOtp(etiqueta, alAcertar) {
    const form = document.querySelector("[data-auth-form]");
    const btn = document.querySelector("[data-auth-submit]");
    if (!form || !btn)
        return;
    form.addEventListener("submit", async (e) => {
        e.preventDefault();
        const campo = form.querySelector("#otp") ?? form.querySelector("#recovery");
        if (!campo)
            return;
...
function protoAuthAgregarAccion(texto, href) {
    const zona = document.querySelector("[data-auth-actions]");
    if (!zona)
        return;
...
const PROTO_AUTH_SCREENS = {
    ...
    "trusted-device": {
        ...
        render: protoRenderTrustedDevice, wire: () => { },
    },
    "select-organization": {
...
function protoInitAuth() {
    ...
    const indice = document.querySelector("[data-auth-index]");
    if (indice) {
        indice.innerHTML = Object.keys(PROTO_AUTH_SCREENS)
            .map((k) => {
