 ...
 * ========================================================================= */
function protoRenderLeadsList(host, user) {
...
function protoRenderLeadRow(row) {
    return `
    <tr data-row-id="${protoEsc(row.leadId)}">
      ...
      <td>${row.productOffering ? protoEsc(row.productOffering) : "<span class='ws-muted'>—</span>"}${row.quantityUnit ? `<br><span class="ws-muted">${protoEsc(row.quantityUnit)}</span>` : ""}</td>
...
function protoWireLeadsList() {
    document.querySelector("[data-leads-sort]")?.addEventListener("change", (e) => {
        const campo = e.currentTarget.value;
    ...
    document.querySelectorAll("[data-open-lead]").forEach((a) => {
 ...
 * ========================================================================= */
function protoSeccion(n, id, titulo, cuerpo) {
    return `
...
function protoRenderCampoCanonico(label, campo, requerido) {
    const valor = campo.status === "CANONICAL" ? protoEsc(String(campo.value))
...
function protoRenderLeadDetail(host, leadId, user) {
    ...
    ${protoSeccion(5, "lead-priority", "Prioridad / Próximo paso", ops
        ? `<dl class="ws-context">
           ...
           <div><dt>Origen de la recomendación</dt><dd>${protoEsc(PROTO_NBA_SOURCE_LABEL[ops.nextBestAction.source])}${ops.nextBestAction.source === "AI_RECOMMENDATION" && ops.nextBestAction.confidence !== null
...
function protoRenderCommunication(c) {
    ...
    <li class="ws-comm ws-comm--${c.direction.toLowerCase()}">
      ...
      ${c.status === "FAILED"
        ? `<p class="ws-comm__failed">Envío fallido (${protoEsc(c.failureCode ?? "sin código")}). No cuenta como respuesta válida${c.availableActions?.includes("RETRY_COMMUNICATION") ? "; se puede reintentar." : "."}</p>` : ""}
    </li>`;
}
function protoWireLeadDetail(leadId, row) {
    const feedback = document.querySelector("[data-lead-feedback]");
    document.querySelectorAll("[data-lead-command]").forEach((btn) => {
        btn.addEventListener("click", async () => {
            const accion = btn.getAttribute("data-lead-command");
            if (!accion || !feedback)
                return;
    ...
    document.querySelector("[data-back-to-list]")?.addEventListener("click", () => {
        protoPatchViewState(PROTO_LEADS_SURFACE, { lastFocusedId: leadId });
    });
    if (row)
        document.title = `${row.leadReference} ${row.companyName} — Workspace (prototipo)`;
}
/* ========================================================================= */
document.addEventListener("DOMContentLoaded", () => {
    if (!protoIsAuthenticated())
        return;
    const host = document.querySelector("[data-leads-surface]");
    const user = protoGetSessionUser();
    if (!host || !user)
        return;
    const leadId = protoUrlParam("leadId");
    if (leadId)
        protoRenderLeadDetail(host, leadId, user);
    else
        protoRenderLeadsList(host, user);
});
