 ...
 */
function protoRenderQual01(host, lead) {
...
function protoRenderQualRevisionEntrada(lead, qual) {
    ...
    const candidatas = ["OPEN_HUMAN_REVIEW", "REQUEST_HUMAN_REVIEW", "REQUEST_MORE_INFORMATION"];
 ...
 * ========================================================================= */
function protoRenderQual02(host, lead, criterionId) {
    const qual = protoGetQualification(lead.leadId);
...
/* ========================================================================= */
function protoRenderQualCabecera(lead, titulo, codigo) {
    return `
...
function protoWireQual(lead) {
    const feedback = document.querySelector("[data-qual-feedback]");
    document.querySelectorAll("[data-qual-command]").forEach((btn) => {
        btn.addEventListener("click", async () => {
            const accion = btn.getAttribute("data-qual-command");
            if (!accion || !feedback)
                return;
...
document.addEventListener("DOMContentLoaded", () => {
    if (!protoIsAuthenticated())
        return;
    const host = document.querySelector("[data-qual-surface]");
    if (!host)
        return;
    ...
    if (criterion)
        protoRenderQual02(host, lead, criterion);
    else
        protoRenderQual01(host, lead);
});
