...
function protoReadStoredScenarioId() {
    try {
...
function protoGetActiveScenarioId() {
    ...
    if (stored && protoGetScenario(stored))
        return stored;
    return PROTO_DEFAULT_SCENARIO_ID;
}
function protoSetActiveScenario(scenarioId) {
    if (!protoGetScenario(scenarioId))
        return;
    try {
 ...
 */
function protoResolveRoute(route, scenarioId) {
...
function protoNavigate(route, scenarioId) {
    if (scenarioId)
...
function protoResetPrototypeState() {
    try {
