...
const PROTO_ASSIGNABLE_TENANT_ROLES = ["TENANT_ADMINISTRATOR"];
/** Roles ofrecibles por BizCap. Catálogo del fixture, no inferencia. */
const PROTO_ASSIGNABLE_ROLES = {
    "LAB-001": [
 ...
 */
function protoAccessChangeSet(userId, tenantId, draft, actorUserId) {
    const actuales = protoBizCapAssignmentsOf(userId, tenantId);
    const actualPorBizCap = new Map(actuales.map((a) => [a.bizCapId, a.roles]));
    const changes = [];
    // BizCaps añadidos o con roles cambiados.
    Object.entries(draft).forEach(([bizCapId, roles]) => {
        if (bizCapId === PROTO_TENANT_ROLE_KEY)
            return; // se trata aparte
        ...
        if (!previos) {
            if (!roles.length)
                return; // seleccionado sin roles: no es un cambio
            changes.push({
                kind: "BIZCAP_ADDED",
                bizCapId: bizCapId,
            ...
            roles.forEach((r) => changes.push({
                kind: "ROLE_ADDED", bizCapId: bizCapId, role: r,
                consequence: `Podrá ejercer ${PROTO_ROLE_LABEL[r]} dentro de ${nombre}.`,
            }));
            return;
        }
        roles.filter((r) => !previos.includes(r)).forEach((r) => changes.push({
            kind: "ROLE_ADDED", bizCapId: bizCapId, role: r,
            consequence: `Añade la función ${PROTO_ROLE_LABEL[r]} en ${nombre}, junto a las que ya tenía.`,
        }));
        previos.filter((r) => !roles.includes(r)).forEach((r) => changes.push({
            kind: "ROLE_REMOVED", bizCapId: bizCapId, role: r,
    ...
    tenantDespues.filter((r) => !tenantAntes.includes(r)).forEach((r) => changes.push({
    ...
    tenantAntes.filter((r) => !tenantDespues.includes(r)).forEach((r) => changes.push({
    ...
    const warnings = [];
    const quedaSinAcceso = Object.entries(draft)
        .filter(([k]) => k !== PROTO_TENANT_ROLE_KEY)
        .every(([, r]) => r.length === 0) && actuales.length > 0;
    if (quedaSinAcceso) {
        warnings.push({
            code: "REMOVES_ALL_ACCESS",
            message: "Con estos cambios la persona conserva su membresía pero se queda sin ninguna BizCap: podrá entrar a la organización y no tendrá nada operativo que hacer.",
            blocking: false,
        });
    }
    ...
    if (eraAdmin && !sigueAdmin && admins.length <= 1) {
        warnings.push({
            ...
            message: "Es la última persona que puede administrar el acceso de esta organización. Si le retiras la administración, nadie podrá volver a conceder accesos, ni siquiera tú.",
            blocking: true,
        });
    }
...
function protoApplyAccessChange(userId, tenantId, draft) {
    const m = protoFindMembership(userId, tenantId);
    if (!m)
        return;
    const previos = new Map(m.bizCaps.map((a) => [a.bizCapId, a]));
...
function protoPermissionsForRoles(roles) {
    const out = new Set();
    roles.forEach((r) => {
        if (r === "SALES_REPRESENTATIVE")
            PROTO_PERMS_SALES_REP.forEach((p) => out.add(p));
        if (r === "HUMAN_REVIEWER")
            PROTO_PERMS_HUMAN_REVIEWER.forEach((p) => out.add(p));
        if (r === "SALES_MANAGER")
            PROTO_PERMS_SALES_MANAGER.forEach((p) => out.add(p));
        if (r === "ORDER_REVIEWER")
            PROTO_PERMS_ORDER_REVIEWER.forEach((p) => out.add(p));
        if (r === "TENANT_ADMINISTRATOR")
            PROTO_PERMS_TENANT_ADMIN.forEach((p) => out.add(p));
        if (r === "SALES_OPERATIONS_ANALYST") {
            ["lead.read", "work.read", "work.manage", "qualification.read", "assignment.read"]
                .forEach((p) => out.add(p));
        }
        if (r === "BIZCAP_ADMINISTRATOR")
            out.add("bizcap.configure");
    });
    return [...out];
}
function protoReadAccessOverrides() {
    try {
...
function protoPersistAccessOverride(userId, tenantId, bizCaps, tenantRoles, tenantPermissions) {
    try {
        const todos = protoReadAccessOverrides().filter((o) => !(o.userId === userId && o.tenantId === tenantId));
 ...
 */
function protoRehydrateAccessOverrides() {
