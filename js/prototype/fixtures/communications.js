...
const PROTO_ENABLED_CHANNELS = [
    "WEB_FORM", "SECURE_CLARIFICATION", "EMAIL", "SYSTEM_MESSAGE",
];
const PROTO_COMMUNICATIONS = [
...
function protoGetCommunications(leadId) {
    return PROTO_COMMUNICATIONS.filter((c) => c.leadId === leadId).sort((a, b) => (b.receivedAt ?? b.dispatchedAt ?? b.createdAt).localeCompare(a.receivedAt ?? a.dispatchedAt ?? a.createdAt));
}
/** Evidencia de primera respuesta válida, si existe. */
function protoGetFirstResponse(leadId) {
    return (PROTO_COMMUNICATIONS.find((c) => c.leadId === leadId && c.isFirstResponse && protoEsPrimeraRespuestaValida(c.status, c.direction)) ?? null);
}
