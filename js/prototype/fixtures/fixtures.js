...
function protoCanon(value, version = 1) {
    return { value, status: "CANONICAL", version };
}
const PROTO_FIELD_MISSING = { value: null, status: "MISSING" };
const PROTO_FIELD_NOT_PROVIDED = { value: null, status: "NOT_PROVIDED" };
const PROTO_CANONICAL_LEADS = [
    {
        leadId: "LEAD-00041",
...
function protoGetQualification(leadId) {
    return PROTO_QUALIFICATIONS[leadId] ?? null;
}
function protoGetFieldComparisons(leadId) {
...
function protoMissingRequiredFields(lead) {
    const faltantes = [];
    const n = lead.commercialNeed;
    if (!lead.contact.email && !lead.contact.phone)
        faltantes.push("contact.emailOrPhone");
    if (n.productOffering.status !== "CANONICAL")
        faltantes.push("productOffering");
    if (n.quantity.status !== "CANONICAL")
        faltantes.push("quantity");
    if (n.unit.status !== "CANONICAL")
        faltantes.push("unit");
    if (n.geographyDestination.status !== "CANONICAL")
        faltantes.push("geographyDestination");
    return faltantes;
}
/**
 ...
 */
function protoLeadPermite(lead, accion) {
