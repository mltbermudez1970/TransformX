...
function protoGetLeadOperations(leadId) {
    return PROTO_LEAD_OPERATIONS.find((o) => o.leadId === leadId) ?? null;
}
function protoLeadRow(leadId) {
    ...
    const cantidad = n.quantity.status === "CANONICAL" && n.unit.status === "CANONICAL"
...
function protoGetSavedView(savedViewId) {
 ...
 */
function protoResolveSavedView(savedViewId, currentUserId, filtros, orden) {
    ...
    PROTO_CANONICAL_LEADS.forEach((lead) => {
        ...
        if (!row)
            return;
        if (view.actorRelative && row.commercialOwner?.userId !== currentUserId)
            return;
        if (!estados.includes(row.lifecycleState))
            return;
        filas.push(row);
    });
    ...
    if (primero) {
        filas.sort((a, b) => {
            ...
            if (primero.field === "priority") {
                ...
                if (primero.direction === "ASC")
                    cmp = -cmp;
            }
            else if (primero.field === "updatedAt") {
                cmp = a.updatedAt.localeCompare(b.updatedAt);
                if (primero.direction === "DESC")
                    cmp = -cmp;
            }
            if (cmp !== 0)
                return cmp;
