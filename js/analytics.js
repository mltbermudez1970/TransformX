 ...
 */
function initControlDeConsentimiento() {
    const host = document.querySelector("[data-consent-control]");
    if (!host)
        return;
    const esWs = esSuperficieDePrototipo();
    ...
    const pintar = () => {
        ...
        const etiqueta = decision === "granted" ? "Aceptada"
         ...
         */
        const acciones = decision === "granted"
      ...
      <div class="consent-control" role="group" aria-labelledby="consent-control-title">
        ...
        host.querySelectorAll("[data-consent-set]").forEach((b) => {
            b.addEventListener("click", () => {
                const v = b.getAttribute("data-consent-set") === "granted" ? "granted" : "denied";
                ...
                if (v === "denied" && anterior === "granted") {
                    const aviso = host.querySelector("[data-consent-feedback]");
                    if (aviso) {
                        aviso.textContent =
                ...
                if (v === "granted")
                    emitirPageview();
                pintar();
                const aviso = document.querySelector("[data-consent-feedback]");
                if (aviso) {
                    aviso.textContent =
                        v === "granted"
