...
/**
 * Igual que `Ga4Window` en el cargador de GA4: los scripts del sitio son
 * globales, no módulos ES, así que `declare global` no aplica. Se declara una
 * interfaz local y se estrecha `window` al usarla.
 */
interface CsWindow {
  /** La invoca `analytics.ts` cuando —y sólo cuando— hay consentimiento. */
  __csActivar?: () => void;
  /** Cola de comandos del tag. La crea el propio script de Contentsquare. */
  _uxa?: unknown[];
}

function initContentsquare(): void {
  const w = window as unknown as CsWindow;
  ...
  if (!/^[0-9a-f]{10,16}$/.test(CS_TAG_ID)) {
    if (location.hostname === "localhost" || location.hostname === "127.0.0.1") {
      ...
      console.warn(
        "[analytics] El tag ID de Contentsquare no tiene forma de hash hexadecimal: " +
  ...
  let activado = false;
  w.__csActivar = (): void => {
    if (activado) return;              // una sola descarga por página
    activado = true;
