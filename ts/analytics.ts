...
interface MixpanelApi {
  ...
  reset(): void;
  /** `track` sustituye la función con la que el SDK emite `$opt_in`. */
 ...
 */
function csActivar(): (() => void) | null {
  const w = window as unknown as { __csActivar?: () => void };
...
function aplicarConsentimiento(v: DecisionDeConsentimiento): void {
  ...
  if (api) {
    try {
       ...
       */
      if (concedido) {
        if (!api.has_opted_in_tracking()) api.opt_in_tracking({ track: () => undefined });
      } else {
