...
function ga4IdEsValido(id: string): boolean {
...
interface Ga4Window {
  ...
  /** Descarga gtag.js y lo configura. Sólo se llama con consentimiento dado. */
  __ga4Activar?: (contexto: Record<string, string | number>) => void;
}
...
function initGa4(): void {
  if (!ga4IdEsValido(GA4_MEASUREMENT_ID)) {
    if (location.hostname === "localhost" || location.hostname === "127.0.0.1") {
      console.warn(
        `[GA4] Measurement ID sin configurar ("${GA4_MEASUREMENT_ID}"). ` +
  ...
  const cola: IArguments[] = w.dataLayer ?? [];
  w.dataLayer = cola;
   ...
   */
  let activado = false;
  w.__ga4Activar = (contexto: Record<string, string | number>): void => {
