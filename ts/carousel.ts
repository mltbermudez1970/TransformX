type CarouselTimer = ReturnType<typeof setInterval> | null;

function initCarousel(): void {
  const carousel = document.querySelector<HTMLElement>(".logo-carousel");
  if (!carousel) return;

  const track = carousel.querySelector<HTMLElement>(".logo-carousel__track");
  const slides = carousel.querySelectorAll<HTMLElement>(".logo-carousel__slide");
  ...
  const tabs = dotsContainer.querySelectorAll<HTMLButtonElement>(".carousel-dot");

  function activate(i: number, focusTab: boolean): void {
    idx = i;
  ...
  dotsEl.addEventListener("keydown", (event: KeyboardEvent): void => {
  ...
  carousel.addEventListener("focusout", (event: FocusEvent): void => {
