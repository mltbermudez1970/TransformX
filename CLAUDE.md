...
- Al tocar meta/OG tags, tener en cuenta la inconsistencia de dominio ya documentada en `PLAN.md` (`sitemap.xml`/`robots.txt` usan `transformx.com`, pero `og:url`/`og:image` en el HTML aún usan `transformingexperiences.com/TransformX/...`) — es una limpieza pendiente a nivel de todas las páginas a la vez, no algo para corregir a medias en una sola página.
