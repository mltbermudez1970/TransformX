...
interface ProtoWorkItemSubject {
  entityType: "LEAD";
  leadId: string;
...
interface ProtoWorkItemActor {
  userId: string;
  displayName: string;
  role?: ProtoRole;
}

interface ProtoWorkItemSla {
  status: "ON_TRACK" | "AT_RISK" | "BREACHED" | "NOT_APPLICABLE";
...
interface ProtoWorkItem {
  ...
  title: string;
  obligation: string;
  subject: ProtoWorkItemSubject;
  assignee: ProtoWorkItemActor | null;
  /** Puede diferir del assignee: son conceptos distintos. */
  commercialOwner?: ProtoWorkItemActor;
  priority: ProtoPriorityCode;
  riskLevel?: "LOW" | "MEDIUM" | "HIGH";
  createdAt: string;
  /** null sólo cuando el fixture declara explícitamente que no hay plazo. */
  dueAt: string | null;
  /** Provisto por el fixture: el frontend no calcula vencimientos. */
  isOverdue: boolean;
  reasonCode: string;
  reasonSummary: string;
  /** Autoritativo para el contexto del WorkItem. */
  availableActions: ProtoAction[];
  targetRoute: string;
  sla: ProtoWorkItemSla;
  version: number;
  updatedAt: string;
  tags?: string[];
}
...
function protoCountOpenWork(leadId: string): number {
