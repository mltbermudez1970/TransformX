...
interface ProtoTenant {
  tenantId: string;
  name: string;
  /** Nombre corto para la insignia de identidad y el conmutador. */
  shortName: string;
  initials: string;
  industry: string;
}

interface ProtoBizCapAssignment {
  bizCapId: ProtoBizCapId;
  /**
   * VARIOS roles simultáneos en el mismo BizCap (§4). El orden es el del
   * fixture: la UI no lo reordena ni elige uno "principal".
   */
  roles: ProtoRole[];
  scopes: ProtoScope[];
  /** Resueltos por el mock de autorización, no derivados del nombre del rol. */
  permissions: ProtoPermission[];
  assignedAt: string;
}

interface ProtoTenantMembership {
  membershipId: string;
  userId: string;
  tenantId: string;
  status: ProtoMembershipStatus;
  /** Vacío = membresía sin BizCaps: estado gobernado, no Workspace falso. */
  bizCaps: ProtoBizCapAssignment[];
  /**
   * Roles que NO son función de un BizCap sino de la organización entera:
   * hoy sólo TENANT_ADMINISTRATOR. Van aparte precisamente porque un rol de
   * BizCap responde "¿qué hace esta persona dentro de esta capacidad?", y
   * administrar el acceso de la organización no es una respuesta a eso.
   */
  tenantRoles: ProtoRole[];
  /** Permisos a nivel de Tenant (administración), aparte de los de BizCap. */
...
const PROTO_TENANTS: ProtoTenant[] = [
  {
...
function protoFindTenant(tenantId: string): ProtoTenant | null {
...
const PROTO_PERMS_SALES_REP: ProtoPermission[] = [
...
const PROTO_PERMS_HUMAN_REVIEWER: ProtoPermission[] = [
  "lead.read", "work.read", "qualification.read",
  "review.read", "review.resolve", "duplicate_review.resolve",
  "communication.read",
];

const PROTO_PERMS_SALES_MANAGER: ProtoPermission[] = [
...
const PROTO_PERMS_ORDER_REVIEWER: ProtoPermission[] = ["order.review"];

const PROTO_PERMS_TENANT_ADMIN: ProtoPermission[] = [
...
const PROTO_MEMBERSHIPS: ProtoTenantMembership[] = [
...
function protoMembershipsOf(userId: string): ProtoTenantMembership[] {
  return PROTO_MEMBERSHIPS.filter((m) => m.userId === userId);
}

/** Sólo las membresías con las que se puede entrar a un Workspace. */
function protoActiveMembershipsOf(userId: string): ProtoTenantMembership[] {
  return protoMembershipsOf(userId).filter((m) => m.status === "active");
}

function protoFindMembership(userId: string, tenantId: string): ProtoTenantMembership | null {
...
function protoBizCapAssignmentsOf(userId: string, tenantId: string): ProtoBizCapAssignment[] {
  const m = protoFindMembership(userId, tenantId);
  if (!m || m.status !== "active") return [];
  return m.bizCaps;
}

function protoBizCapAssignment(
  userId: string,
  tenantId: string,
  bizCapId: string
): ProtoBizCapAssignment | null {
  return protoBizCapAssignmentsOf(userId, tenantId).find((b) => b.bizCapId === bizCapId) ?? null;
}

/**
 * Acceso efectivo del usuario en un Tenant: la unión de lo que le dan sus
 * asignaciones de BizCap más los permisos de Tenant.
 *
 * Es una **proyección de lectura** de los fixtures, no un cálculo de reglas:
 * la enmienda §8 exige que los permisos efectivos se presenten como derivados
 * y de sólo lectura.
 */
interface ProtoEffectiveAccess {
  userId: string;
  tenantId: string;
  status: ProtoMembershipStatus | "none";
  bizCapIds: ProtoBizCapId[];
  /** Todos los roles del usuario en el Tenant, sin duplicar, con su BizCap. */
  rolesByBizCap: { bizCapId: ProtoBizCapId; roles: ProtoRole[] }[];
  roles: ProtoRole[];
  scopes: ProtoScope[];
  permissions: ProtoPermission[];
}

function protoEffectiveAccess(userId: string, tenantId: string): ProtoEffectiveAccess {
  const m = protoFindMembership(userId, tenantId);
  ...
  const roles = new Set<ProtoRole>();
  const scopes = new Set<ProtoScope>();
  const permissions = new Set<ProtoPermission>();
  m.bizCaps.forEach((b) => {
...
function protoTienePermisoEnTenant(
  userId: string,
  tenantId: string,
  permiso: ProtoPermission
): boolean {
  return protoEffectiveAccess(userId, tenantId).permissions.includes(permiso);
}

/** ¿El usuario puede operar ese BizCap en ese Tenant? */
function protoTieneBizCap(userId: string, tenantId: string, bizCapId: string): boolean {
...
function protoRolesLabel(roles: ProtoRole[]): string {
 ...
 */
function protoTenantAdminsOf(tenantId: string): ProtoTenantMembership[] {
  return PROTO_MEMBERSHIPS.filter(
    (m) =>
      m.tenantId === tenantId &&
      m.status === "active" &&
...
function protoTenantMembers(tenantId: string): ProtoTenantMembership[] {
