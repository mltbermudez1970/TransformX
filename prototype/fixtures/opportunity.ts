...
interface ProtoReadinessEvidence {
...
interface ProtoReadinessCondition {
  conditionId: ProtoReadinessConditionId;
  name: string;
  status: ProtoReadinessStatus;
  blockingWhenUnsatisfied: boolean;
  question: string;
  summary: string;
  evidence: ProtoReadinessEvidence[];
  /** Ruta a la superficie que resuelve la condición. */
  route: { routeId: string; params?: Record<string, string>; label: string } | null;
  /** Presente sólo en RC-08 cuando hay una oportunidad abierta. */
  relatedOpportunityRef?: string;
  policy: { policyVersion: string; evaluatedAt: string };
}

interface ProtoOpportunityReadiness {
  leadId: string;
  /** Autoritativo. El frontend no lo deriva. */
  status: ProtoOverallReadiness;
  evaluatedAt: string;
  policyVersion: string;
  conditions: ProtoReadinessCondition[];
  blockingConditionIds: ProtoReadinessConditionId[];
  availableActions: ProtoAction[];
}

/* ---------------------------------------------------------------------------
 * §5.1 — Modelo compartido de referencia a oportunidad (PM-06 y REV-03)
 * ------------------------------------------------------------------------- */

interface ProtoOpportunityReferenceSummary {
  opportunityId: string;
  opportunityReference: string;
  status: ProtoOpportunityStatus;
  sourceLeadId: string;
  companyId: string;
  companyName: string;
  commercialNeedId: string;
  commercialNeedVersion: number;
  owner: { userId: string; displayName: string };
  createdAt: string;
}

/** Oportunidad abierta preexistente, usada por RC-08 y por REV-03. */
const PROTO_OPEN_OPPORTUNITIES: ProtoOpportunityReferenceSummary[] = [
  {
...
function protoFindOpenOpportunity(ref: string): ProtoOpportunityReferenceSummary | null {
...
function protoRelatedOpenOpportunity(leadId: string): ProtoOpportunityReferenceSummary | null {
...
function protoRc(
  ...
  summary: string,
  evidence: ProtoReadinessEvidence[] = [],
  route: ProtoReadinessCondition["route"] = null,
  evaluatedAt = "2026-09-05T10:30:00-05:00"
): ProtoReadinessCondition {
...
const PROTO_READINESS: ProtoOpportunityReadiness[] = [
  {
    ...
    conditions: [
      protoRc("RC-01", "SATISFIED", "Calificación completada con resultado aceptable.",
        [{ type: "SYSTEM_ASSESSMENT", ref: "QA-002", label: "Evaluación", value: "PASS v2" }]),
      protoRc("RC-02", "SATISFIED", "Prioridad disponible (Baja, anulación humana registrada).",
        [{ type: "SYSTEM_ASSESSMENT", ref: "priority-policy-v3", label: "Prioridad", value: "LOW" }]),
      protoRc("RC-03", "SATISFIED", "Responsable comercial asignado.",
        [{ type: "SYSTEM_ASSESSMENT", ref: "assignment.current", label: "Responsable", value: "Elena Mora" }]),
      protoRc("RC-04", "SATISFIED", "Próximo paso gobernado disponible.",
        [{ type: "SYSTEM_ASSESSMENT", ref: "nba", label: "NBA", value: "VIEW_OPPORTUNITY_READINESS" }]),
      protoRc("RC-05", "SATISFIED", "Necesidad comercial definida sin ambigüedad material."),
      protoRc("RC-06", "SATISFIED", "Contacto, producto, cantidad, unidad y destino son canónicos.",
        [{ type: "CANONICAL_FACT", ref: "NEED-0047", label: "Necesidad", value: "versión 2" }]),
      protoRc("RC-07", "BLOCKING", "Hay una revisión de readiness abierta que debe resolverse antes de convertir.",
        [{ type: "SYSTEM_ASSESSMENT", ref: "WI-0047", label: "WorkItem abierto", value: "OPPORTUNITY_READINESS_REVIEW" }],
        { routeId: "lab-001-reviews", label: "Resolver la revisión abierta" }),
  ...
  },
  {
    ...
    conditions: [
      protoRc("RC-01", "SATISFIED", "Calificación completada; QD-02 cubierto por excepción aprobada.",
        [{ type: "SYSTEM_ASSESSMENT", ref: "QA-003", label: "Evaluación", value: "PASS v4 (con excepción)" }]),
      protoRc("RC-02", "SATISFIED", "Prioridad alta disponible.",
        [{ type: "SYSTEM_ASSESSMENT", ref: "priority-policy-v3", label: "Prioridad", value: "HIGH" }]),
      protoRc("RC-03", "SATISFIED", "Responsable comercial asignado.",
        [{ type: "SYSTEM_ASSESSMENT", ref: "assignment.current", label: "Responsable", value: "Elena Mora" }]),
      protoRc("RC-04", "SATISFIED", "Próximo paso gobernado disponible."),
      protoRc("RC-05", "SATISFIED", "Necesidad comercial definida sin ambigüedad material."),
      protoRc("RC-06", "SATISFIED", "Todos los hechos comerciales requeridos son canónicos.",
        [{ type: "CANONICAL_FACT", ref: "NEED-0048", label: "Necesidad", value: "versión 3" }]),
  ...
  },
  {
    ...
    availableActions: ["VIEW_OPPORTUNITY_READINESS"],
    conditions: [
      protoRc("RC-01", "PENDING", "La calificación sigue en evaluación: QD-03 está en revisión humana.",
        [{ type: "SYSTEM_ASSESSMENT", ref: "QA-001", label: "Evaluación", value: "REVIEW v3" }],
        { routeId: "lab-001-qualification", label: "Ver calificación" }),
      ...
      protoRc("RC-07", "PENDING", "Hay una revisión humana abierta cuyo resultado aún no se conoce.",
        [{ type: "SYSTEM_ASSESSMENT", ref: "HR-0045", label: "Revisión", value: "OPEN" }],
        { routeId: "lab-001-reviews", label: "Abrir la revisión" }),
      protoRc("RC-08", "SATISFIED", "No existe otra oportunidad abierta para esta necesidad."),
    ],
  },
  {
    ...
    availableActions: ["VIEW_OPPORTUNITY_READINESS"],
    conditions: [
      protoRc("RC-01", "BLOCKING", "La calificación no se ha iniciado para este registro.",
        [], { routeId: "lab-001-qualification", label: "Ver calificación" }),
      ...
      protoRc("RC-05", "SATISFIED", "Necesidad comercial definida."),
      protoRc("RC-06", "SATISFIED", "Hechos comerciales requeridos completos."),
      protoRc("RC-07", "BLOCKING", "Hay una revisión de duplicado abierta.",
        [{ type: "SYSTEM_ASSESSMENT", ref: "DR-0044", label: "Revisión", value: "OPEN" }],
        { routeId: "lab-001-reviews", label: "Abrir la revisión de duplicado" }),
      { ...protoRc("RC-08", "BLOCKING", "Ya existe una oportunidad abierta para la misma necesidad comercial.",
          [{ type: "SYSTEM_ASSESSMENT", ref: "OPP-2026-00041", label: "Oportunidad", value: "OPEN" }],
          { routeId: "lab-001-opportunity", params: { opportunityId: "OPP-2026-00041" }, label: "Ver oportunidad existente" }),
        relatedOpportunityRef: "OPP-2026-00041" },
    ],
  },
];

function protoGetReadiness(leadId: string): ProtoOpportunityReadiness | null {
  return PROTO_READINESS.find((r) => r.leadId === leadId) ?? null;
}

/** Conteo de presentación. No es el estado autoritativo. */
function protoReadinessCounts(r: ProtoOpportunityReadiness): { satisfechas: number; total: number } {
...
/* ---------------------------------------------------------------------------
 * §6 — Snapshot inmutable + §4 oportunidad creada
 * ------------------------------------------------------------------------- */

interface ProtoReadinessSnapshot {
  snapshotId: string;
  capturedAt: string;
  overallStatus: ProtoOverallReadiness;
  policyVersion: string;
  bizCapDefinitionId: string;
  bizCapInstanceId: string;
  configurationVersion: string;
  leadId: string;
  leadVersion: number;
  commercialNeedId: string;
  commercialNeedVersion: number;
  /** TODAS las condiciones evaluadas, incluidas SATISFIED y NOT_APPLICABLE. */
  conditions: { conditionId: ProtoReadinessConditionId; name: string; status: ProtoReadinessStatus; summary: string; evidenceRefs: string[] }[];
  blockingConditionIds: ProtoReadinessConditionId[];
}

interface ProtoQualifiedOpportunity {
  opportunityId: string;
  opportunityReference: string;
  status: ProtoOpportunityStatus;
  sourceLead: { leadId: string; leadReference: string };
  company: { companyId: string; companyName: string };
  commercialNeed: { commercialNeedId: string; commercialNeedVersion: number };
  owner: { userId: string; displayName: string };
  createdAt: string;
  readinessSnapshot: ProtoReadinessSnapshot;
  nextCommercialStep: {
    code: ProtoNextCommercialStepCode;
    label: string;
    targetCapability: string;
    targetCapabilityName: string;
    /** Null: LAB-002 queda fuera del alcance operativo de LAB-001. */
    route: null;
    status: "HANDOFF_ONLY";
  };
}
...
function protoReadCreatedOpportunities(): ProtoQualifiedOpportunity[] {
  try {
    const raw = sessionStorage.getItem(PROTO_CREATED_OPP_KEY);
    return raw ? (JSON.parse(raw) as ProtoQualifiedOpportunity[]) : [];
  } catch {
    return [];
  }
}

function protoWriteCreatedOpportunities(list: ProtoQualifiedOpportunity[]): void {
  try {
...
const PROTO_CREATED_OPPORTUNITIES: ProtoQualifiedOpportunity[] = protoReadCreatedOpportunities();

function protoBuildSnapshot(r: ProtoOpportunityReadiness, lead: ProtoCanonicalLead): ProtoReadinessSnapshot {
  return {
...
function protoCreateOpportunity(lead: ProtoCanonicalLead, r: ProtoOpportunityReadiness): ProtoQualifiedOpportunity {
  const ops = protoGetLeadOperations(lead.leadId);
  const ref = `OPP-2026-${lead.leadId.slice(-5)}`;
  const opp: ProtoQualifiedOpportunity = {
    opportunityId: ref,
...
function protoFindCreatedOpportunity(leadId: string): ProtoQualifiedOpportunity | null {
...
function protoResetCreatedOpportunities(): void {
  try {
...
const PROTO_CONCURRENT_ACTOR = {
  ...
  role: "SALES_MANAGER" as ProtoRole,
  scenarioPermission: "opportunity.convert" as ProtoPermission,
};

/** availableActions de LEAD-00048 tras la conversión concurrente (§9.3). */
const PROTO_POST_CONFLICT_ACTIONS: ProtoAction[] = [
