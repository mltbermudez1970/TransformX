...
function protoReadStoredMode(): boolean {
  try {
...
function protoPersistMode(active: boolean): void {
  try {
    if (active) {
 ...
 */
function protoWithPrototypeParam(href: string): string {
  if (href.includes(`${PROTO_MODE_PARAM}=`)) return href;
 ...
 */
function initPrototypeMode(): void {
  if (!protoIsPrototypeMode()) return;

  document.documentElement.setAttribute("data-prototype", "true");

  document
    .querySelectorAll<HTMLElement>("[data-prototype-entry]")
    .forEach((element) => {
