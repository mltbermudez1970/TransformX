 ...
 */
const PROTO_TENANT_SCOPED_KEYS: string[] = [
...
const PROTO_TENANT_SCOPED_PREFIXES: string[] = ["transformx-view-"];
 ...
 */
function protoClearTenantScopedState(): void {
  try {
    PROTO_TENANT_SCOPED_KEYS.forEach((k) => sessionStorage.removeItem(k));
    const aBorrar: string[] = [];
    for (let i = 0; i < sessionStorage.length; i++) {
      const k = sessionStorage.key(i);
      if (k && PROTO_TENANT_SCOPED_PREFIXES.some((p) => k.startsWith(p))) aBorrar.push(k);
    }
 ...
 */
function protoSwitchTenant(tenantId: string): string | null {
  ...
  if (!protoSetActiveTenant(tenantId)) return null;
 ...
 */
function protoSafeRouteForTenant(routeId: string, userId: string, tenantId: string): string {
  const bizCapId = protoBizCapOfRoute(routeId);
  if (!bizCapId) return protoRouteHref("home");
  if (protoTieneBizCap(userId, tenantId, bizCapId)) return protoRouteHref(routeId);
 ...
 */
function protoBizCapOfRoute(routeId: string): ProtoBizCapId | null {
  const route = protoFindRoute(routeId);
  if (!route) return null;
  const cadena = [...route.parents, route.routeId];
  if (cadena.includes("lab-001")) return "LAB-001";
  return null;
}
 ...
 */
function protoTenantGuardRedirect(): string | null {
